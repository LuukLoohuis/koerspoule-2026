...
2. directeursAnalyse: 9–12 zinnen, 1100–1500 tekens. Begint met het cijfer in tekst ("Vijf komma één.", "Zeven en een half.", "Acht komma twee."). Bouw een echte analyse op in meerdere beats: (a) het oordeel + cijfer, (b) wat goed/sterk was met concrete namen, (c) wat zwak/risicovol was met concrete namen/percentages, (d) het joker-resultaat en de trend, (e) een vooruitblik/waarschuwing, (f) een mokerslag-slotzin. Bevat minstens 3 Lefevere-woorden uit de woordenschat. Bevat MEERDERE concrete observaties uit de input (renner-namen, percentages, joker-resultaat, trend) — benut de ruimte om streng en onderbouwd te oordelen, niet samenvatten maar uitdiepen. Spreekt deelnemer aan in 2e persoon ("ge"/"uw"). Max 2 uitroeptekens. Bevat minstens drie QUOTABELE boutades, en de slotzin is een mokerslag (zie PUNCH & HUMOR) — geen brave afronding. Toon: ambetant, streng, droog-grappig.
...
type LefevereResult = { directeursAnalyse: string; ploegKarakterisering: string };
...
function parseReport(text: string): LefevereResult | null {
...
async function callOpenAI(userPrompt: string): Promise<LefevereResult> {
  ...
  const parsed2 = parseReport(r2.text);
  if (!parsed2) {
