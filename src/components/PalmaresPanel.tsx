...
function rankTone(rank: number) {
  if (rank === 1) return "border-(--medal-gold) bg-[#f2c955] text-(--ink-sepia)";
  if (rank === 2) return "border-(--medal-silver) bg-[#d8d6d0] text-(--ink-sepia)";
  if (rank === 3) return "border-(--medal-bronze) bg-[#ca8a4b] text-white";
  return "border-[#6b5640]/25 bg-[#fffdf7] text-(--ink-sepia)";
}
...
function LaurelWreath() {
  ...
  return (
    <svg viewBox="0 0 340 210" className="absolute inset-0 h-full w-full text-(--palmares-accent)" aria-hidden>
...
function MiniLaurel() {
  return (
    <svg viewBox="0 0 58 58" className="h-12 w-12 text-(--palmares-accent)" fill="none" aria-hidden>
...
function AchievementStat({ icon, value, label, className }: { icon: ReactNode; value: number; label: string; className?: string }) {
  ...
  return (
    <div className={cn("flex min-h-[88px] items-center gap-3 px-4 py-3.5 sm:min-h-[98px] sm:justify-center sm:px-5", className)}>
      <span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-full border bg-[#fffdf8] shadow-[0_3px_9px_rgba(58,42,26,0.07)] sm:h-12 sm:w-12", achieved ? "border-(--palmares-accent) text-(--palmares-accent-strong)" : "border-[#6b5640]/15 text-[#6b5640]/55")}>{icon}</span>
      <span><strong className={cn("block font-oswald text-3xl font-black leading-none", achieved ? "text-(--palmares-accent-strong)" : "text-(--ink-sepia)")}>{value}</strong><span className="mt-1 block text-xs font-medium leading-tight text-(--ink-faded)">{label}</span><span className={cn("mt-1.5 block h-px w-8", achieved ? "bg-(--palmares-accent)" : "bg-[#6b5640]/18")} aria-hidden /></span>
    </div>
  );
}
function StageWinRow({ win }: { win: StageDagzege }) {
  const { t } = useTranslation();
  return <div className="flex items-center gap-3 rounded-xl border border-(--palmares-accent)/60 bg-(--palmares-accent-soft) px-3 py-2.5"><Trophy className="h-4 w-4 shrink-0 text-(--palmares-accent-strong)" aria-hidden /><div className="min-w-0 flex-1"><span className="font-oswald text-sm font-bold text-(--ink-sepia)">{t("common.palmares.stageLabel", { n: win.stage_number })}</span>{win.stage_name && <span className="ml-1.5 text-xs text-(--ink-faded)">· {win.stage_name}</span>}</div><span className="shrink-0 font-oswald text-sm font-bold text-(--palmares-accent-strong)">{t("common.palmares.pt", { n: win.points })}</span></div>;
}
...
function SubpouleCard({ subpoule }: { subpoule: PalmaresSubpoule }) {
  ...
  return (
    <article className="relative grid grid-cols-[52px_minmax(0,1fr)_auto] items-center gap-3 overflow-hidden rounded-2xl border border-[#6b5640]/16 bg-[#fffdf8]/92 p-4 shadow-[0_4px_12px_rgba(58,42,26,0.065)]" style={raceThemeStyle(theme)}>
      <span className="absolute inset-y-0 left-0 w-1 bg-(--palmares-accent)" aria-hidden /><div className={cn("vintage-medal flex h-12 w-12 items-center justify-center rounded-full border-2 font-oswald text-lg font-black", rankTone(subpoule.my_rank))}>#{subpoule.my_rank || "—"}</div>
      <div className="min-w-0"><h3 className="truncate font-oswald text-base font-black uppercase text-(--ink-sepia)">{subpoule.subpoule_name}</h3><p className="mt-1 flex items-center gap-1.5 truncate text-xs text-(--ink-faded)"><FlagIcon country={gameTypeToCountry(subpoule.game_type)} className="h-3 w-4 rounded-sm object-cover" /><span className="truncate">{subpoule.game_name}</span></p><p className="mt-2 text-[11px] text-(--ink-faded)">{subpoule.stage_wins} {t("common.palmares.stageWins").toLowerCase()} · {subpoule.stage_podiums} {t("common.palmares.stagePodiums").toLowerCase()}</p></div>
      <div className="text-right">{topThree && <Medal className="mb-1 ml-auto h-4 w-4 text-(--palmares-accent-strong)" aria-hidden />}<strong className={cn("font-oswald text-xl font-black", topThree ? "text-(--palmares-accent-strong)" : "text-(--ink-sepia)")}>#{subpoule.my_rank || "—"}</strong><span className="block text-[11px] text-(--ink-faded)">{t("common.palmares.of", { total: subpoule.total_members.toLocaleString(i18n.language) })}</span></div>
    </article>
  );
}

function PalmaresSkeleton() { return <div className="space-y-4"><div className="h-128 animate-pulse rounded-3xl bg-secondary/50" /><div className="h-40 animate-pulse rounded-2xl bg-secondary/50" /></div>; }
...
export default function PalmaresPanel() {
  ...
  return (
    <section className="vintage-paper relative overflow-hidden rounded-[1.75rem] border border-[#6b5640]/18 p-4 text-(--ink-sepia) shadow-[0_12px_34px_rgba(58,42,26,0.1)] sm:p-6 lg:p-8" style={raceThemeStyle(bestTheme)}>
      ...
      <div className="relative rounded-[1.4rem] border border-[#6b5640]/10 bg-[#fffaf0]/40 px-4 pb-5 pt-5 sm:px-6 sm:pb-7 sm:pt-6 lg:px-8 lg:pb-8 lg:pt-7">
        <header className="relative min-h-[180px] overflow-hidden sm:min-h-[205px] lg:min-h-[220px]">
          <HeaderCyclingScene />
          <div className="relative z-10 flex items-center gap-3 font-oswald text-[10px] font-bold uppercase tracking-[0.28em] text-(--palmares-accent-strong) sm:text-xs"><span className="h-px w-8 bg-current sm:w-11" aria-hidden />{t("common.palmares.personalHonours")}<span className="hidden h-px w-8 bg-current sm:block sm:w-11" aria-hidden /><span className="text-lg leading-none" aria-hidden>❧</span></div>
          ...
          <p className="relative z-10 mt-4 font-serif text-base italic text-(--ink-faded) sm:text-xl">{t("common.palmares.heroSub")}</p>
          <button type="button" onClick={sharePalmares} className="relative z-20 mt-5 inline-flex h-11 items-center gap-2 rounded-xl border border-[#4b3825]/70 bg-[#fffdf8]/80 px-4 font-sans text-sm font-bold text-(--ink-sepia) shadow-[0_3px_8px_rgba(58,42,26,0.08)] transition-colors hover:bg-white focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-(--palmares-accent) sm:absolute sm:right-0 sm:top-0 sm:mt-0 sm:h-12 sm:px-5 sm:text-base"><Share2 className="h-4 w-4" aria-hidden />{t("common.palmares.share")}</button>
        </header>

        <article className="relative grid overflow-hidden rounded-[1.4rem] border border-(--palmares-accent) bg-[#fffdf8]/88 shadow-[0_8px_22px_rgba(58,42,26,0.085)] lg:grid-cols-[39fr_61fr]">
          <div className="relative flex min-h-[235px] items-center justify-center overflow-hidden [background:linear-gradient(145deg,var(--palmares-hero-start),var(--palmares-hero-end))] px-5 py-6 sm:min-h-[255px]">
            ...
            <div className="relative z-10 text-center"><span className="inline-flex rounded-lg border border-(--palmares-accent) bg-[#fffaf0]/88 px-4 py-1.5 font-oswald text-[10px] font-bold uppercase tracking-[0.17em] text-(--palmares-accent-strong) shadow-xs sm:text-xs">{t("common.palmares.bestFinalRank")}</span><strong className="mt-3 block font-oswald text-[clamp(4.25rem,8vw,6rem)] font-black leading-none tracking-[-0.055em] text-[#2a190b]">#{bestGame?.my_rank || "—"}</strong><span className="mx-auto mt-1.5 flex h-12 w-12 items-center justify-center rounded-full border border-(--palmares-accent) bg-[#fffdf8] shadow-[0_3px_9px_rgba(58,42,26,.12)]"><img loading="lazy" decoding="async" src={JERSEY_BY_THEME[bestTheme.key]} alt="" className="h-9 w-auto object-contain" /></span></div>
            <div className="pointer-events-none absolute -right-10 -top-4 hidden h-[calc(100%+2rem)] w-20 rounded-[50%] border-r-2 border-(--palmares-accent) bg-[#fffdf8] lg:block" aria-hidden />
          </div>
          <div className="relative flex min-h-[235px] flex-col justify-center px-6 py-6 sm:min-h-[255px] sm:px-9 lg:pl-12 lg:pr-9">
            <p className="font-oswald text-xs font-bold uppercase tracking-[0.18em] text-(--palmares-accent-strong) sm:text-sm">{raceHeading(bestGame)}</p><h3 className="mt-3 font-oswald text-3xl font-black uppercase leading-[1.02] tracking-[-0.02em] text-[#281a0e] sm:text-4xl lg:text-[2.65rem]">{t("common.palmares.strongestGrandTour")}</h3>
            <p className="mt-4 flex items-center gap-2 text-sm text-(--ink-faded) sm:text-base"><Users className="h-5 w-5 text-[#8f7d62]/70" aria-hidden />{bestGame ? t("common.palmares.rankOf", { rank: bestGame.my_rank, total: bestGame.total_participants.toLocaleString(i18n.language) }) : "—"}</p>
            {bestPercent !== null && <div className="mt-4 flex w-full flex-wrap items-center gap-x-3 gap-y-1 rounded-xl border border-[#6b5640]/16 bg-[#fffdf8]/90 px-4 py-2.5 shadow-xs sm:w-fit"><BarChart3 className="h-5 w-5 shrink-0 text-(--palmares-accent)" aria-hidden /><strong className="text-sm text-(--ink-sepia) sm:text-base">{t("common.palmares.topPercent", { percent: formatPercentage(bestPercent, i18n.language) })}</strong><span className="text-xs text-(--ink-faded) sm:text-sm">{t("common.palmares.ofField")}</span></div>}
          </div>
        </article>
        ...
        <Tabs defaultValue="races" className="mt-7">
          <div className="flex items-end justify-between gap-4 border-b border-[#6b5640]/18"><TabsList className="h-auto gap-1 rounded-none bg-transparent p-0"><TabsTrigger value="races" className="rounded-t-xl rounded-b-none border border-transparent px-4 py-3 font-sans text-sm font-bold text-(--ink-faded) data-[state=active]:border-[#6b5640]/18 data-[state=active]:border-b-2 data-[state=active]:border-b-(--palmares-accent) data-[state=active]:bg-[#fffdf8]/85 data-[state=active]:text-(--ink-sepia) data-[state=active]:shadow-none sm:px-6">{t("common.palmares.racesTab")}</TabsTrigger><TabsTrigger value="subpoules" className="rounded-t-xl rounded-b-none border border-transparent px-4 py-3 font-sans text-sm font-bold text-(--ink-faded) data-[state=active]:border-[#6b5640]/18 data-[state=active]:border-b-2 data-[state=active]:border-b-(--palmares-accent) data-[state=active]:bg-[#fffdf8]/85 data-[state=active]:text-(--ink-sepia) data-[state=active]:shadow-none sm:px-6">{t("common.palmares.subpoules")}</TabsTrigger></TabsList>{latestSeason && <span className="hidden pb-3 font-oswald text-xs font-bold uppercase tracking-[0.16em] text-(--ink-faded) sm:block">{latestSeason.year ?? t("common.palmares.earlier")} · {t("common.palmares.chapters", { count: latestSeason.games.length })}</span>}</div>
          <TabsContent value="races" className="mt-5 space-y-7">
            {seasons.map(({ year, games: seasonGames }, seasonIndex) => <section key={year ?? "past"}>{(seasons.length > 1 || seasonIndex > 0) && <div className="mb-3 flex items-center gap-4"><h3 className="font-oswald text-sm font-black uppercase tracking-[0.16em] text-(--ink-faded)">{year ?? t("common.palmares.earlier")}</h3><span className="h-px flex-1 bg-[#6b5640]/16" aria-hidden /></div>}<div className="relative ml-2 space-y-3 border-l border-[#6b5640]/22 pl-5 sm:ml-0 sm:pl-8">{seasonGames.map((game) => <div key={game.game_id} className="relative"><span className="absolute left-[-1.78rem] top-7 h-3 w-3 rounded-full border-2 border-[#fffaf0] bg-(--palmares-accent) shadow-[0_0_0_1px_var(--palmares-accent)] sm:left-[-2.32rem]" style={raceThemeStyle(getRaceTheme(game.game_type))} aria-hidden /><GameCard game={game} defaultOpen={false} /></div>)}</div></section>)}
          </TabsContent>
          <TabsContent value="subpoules" className="mt-5">{subpoules.length > 0 ? <div className="grid gap-3 lg:grid-cols-2">{subpoules.map((subpoule) => <SubpouleCard key={subpoule.subpoule_id} subpoule={subpoule} />)}</div> : <div className="rounded-2xl border border-[#6b5640]/16 bg-[#fffdf8]/70 p-8 text-center"><Users className="mx-auto mb-3 h-9 w-9 text-[#6b5640]/50" aria-hidden /><p className="font-serif text-sm italic text-(--ink-faded)">{t("common.palmares.noSubpoules")}</p></div>}</TabsContent>
        </Tabs>

        <footer className="mt-8 flex items-center gap-4 rounded-xl border border-(--palmares-accent)/35 bg-(--palmares-accent-soft)/55 px-5 py-4 sm:px-6">
          <MiniLaurel />
          <p className="min-w-0 text-sm leading-relaxed text-(--ink-faded)">
            <strong className="block font-medium text-(--palmares-accent-strong)">{t(`common.palmares.themeOutro.${bestTheme.key}.title`)}</strong>
