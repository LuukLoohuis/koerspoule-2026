...
export default function AdminV3() {
  ...
  async function loadGames() {
    ...
    if ((data ?? []).length && !activeGameId) {
      // Standaard: de lopende game (niet afgerond) — inschrijving open, sneak
      // preview of live. Handmatige keuze blijft (alleen zetten als leeg).
      const list = (data ?? []) as Game[];
      const afgerond = (st?: string | null) => st === "finished" || st === "closed";
      const preferred = list.find((g) => !afgerond(g.status)) ?? list[0];
  ...
  async function loadGameScoped(gameId: string) {
    ...
    const [c, t, r, s] = await Promise.all([
      ...
      supabase
        .from("riders")
        .select("id, name, start_number, team_id, is_youth_eligible, is_dnf, is_vervallen, firstcycling_id, teams(name)")
        .eq("game_id", gameId)
        .order("start_number", { nullsFirst: false }),
      supabase
        .from("stages")
        .select("id, game_id, stage_number, name, date, status, stage_type, distance_km, profile_image_url, profile_data")
        .eq("game_id", gameId)
        .order("stage_number"),
    ]);
    ...
    setRiders(
      ((r.data ?? []) as unknown as Array<{
        ...
        is_vervallen: Boolean(row.is_vervallen),
